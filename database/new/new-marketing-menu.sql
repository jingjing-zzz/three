...
WHERE parent_permission.`menu_id` = 2397 AND parent_permission.`deleted` = b'0'
  AND NOT EXISTS (
    ...
    WHERE existing_permission.`role_id` = parent_permission.`role_id`
      AND existing_permission.`menu_id` = marketing_menu.`id`

...
FROM DUAL WHERE @parent_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM system_menu WHERE permission='crm:statistics-forecast:query');
...
WHERE permission='crm:statistics-forecast:query' AND @parent_id IS NOT NULL;

SET @menu_id := (SELECT id FROM system_menu WHERE permission='crm:statistics-forecast:query' AND deleted=b'0' LIMIT 1);
...
SELECT id,parent_id,path,component,component_name,permission FROM system_menu WHERE permission='crm:statistics-forecast:query';

...
SET @demo_erp_customer_id = (
  SELECT `id` FROM `erp_customer`
  WHERE `name` = '星云科技（演示客户）' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0'
  ORDER BY `id` LIMIT 1
);

INSERT INTO `erp_supplier` (`name`, `contact`, `mobile`, `remark`, `status`, `sort`,
                            `creator`, `create_time`, `updater`, `update_time`, `deleted`, `tenant_id`)
SELECT '云服务供应商（演示）', '周经理', '13800000003', '财务分析演示供应商', 0, 1,
       @demo_user_id, NOW(), @demo_user_id, NOW(), b'0', @demo_tenant_id
WHERE NOT EXISTS (
  SELECT 1 FROM `erp_supplier`
  WHERE `name` = '云服务供应商（演示）' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0'
);
SET @demo_supplier_id = (
  SELECT `id` FROM `erp_supplier`
  WHERE `name` = '云服务供应商（演示）' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0'
  ORDER BY `id` LIMIT 1
);
...
-- 财务分析除四类单据外，还汇总已审核收款、付款；补齐两类数据使分析页所有卡片可验证。
INSERT INTO `erp_finance_receipt`
(`no`, `status`, `receipt_time`, `finance_user_id`, `customer_id`, `account_id`,
 `total_price`, `discount_price`, `receipt_price`, `remark`, `creator`, `create_time`,
 `updater`, `update_time`, `deleted`, `tenant_id`)
SELECT 'DEMO-SKD-001', 20, DATE_SUB(NOW(), INTERVAL 6 DAY), @demo_user_id, @demo_erp_customer_id, @demo_account_id,
       50000.000000, 2000.000000, 48000.000000, '演示收款：年度服务首期款', @demo_user_id, NOW(), @demo_user_id, NOW(), b'0', @demo_tenant_id
WHERE @demo_erp_customer_id IS NOT NULL AND @demo_account_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM `erp_finance_receipt` WHERE `no` = 'DEMO-SKD-001' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0');

INSERT INTO `erp_finance_payment`
(`no`, `status`, `payment_time`, `finance_user_id`, `supplier_id`, `account_id`,
 `total_price`, `discount_price`, `payment_price`, `remark`, `creator`, `create_time`,
 `updater`, `update_time`, `deleted`, `tenant_id`)
SELECT 'DEMO-FKD-001', 20, DATE_SUB(NOW(), INTERVAL 4 DAY), @demo_user_id, @demo_supplier_id, @demo_account_id,
       12000.000000, 0.000000, 12000.000000, '演示付款：云资源订阅服务费', @demo_user_id, NOW(), @demo_user_id, NOW(), b'0', @demo_tenant_id
WHERE @demo_supplier_id IS NOT NULL AND @demo_account_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM `erp_finance_payment` WHERE `no` = 'DEMO-FKD-001' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0');

SELECT @demo_tenant_id AS `演示租户`, @demo_user_id AS `演示用户`, @demo_account_id AS `演示账户`,
       (SELECT COUNT(*) FROM `erp_finance_record` WHERE `no` LIKE 'DEMO-%' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0') AS `财务单据数`,
       (SELECT COUNT(*) FROM `erp_finance_receipt` WHERE `no` LIKE 'DEMO-%' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0') AS `收款单数`,
       (SELECT COUNT(*) FROM `erp_finance_payment` WHERE `no` LIKE 'DEMO-%' AND `tenant_id` = @demo_tenant_id AND `deleted` = b'0') AS `付款单数`;

  ...
  <ContentWrap :bodyStyle="{ padding: '10px 20px 0' }" class="position-relative">
    <div class="processInstance-wrap-main">
      <el-scrollbar>
        ...
        />
        <div class="flex">
          <div class="text-#878c93 h-15px">{{ t('process.instance.id') }}：{{ id }}</div>
        ...
        <div class="flex items-center gap-5 mb-10px text-13px h-35px">
          ...
          <div class="text-#878c93"> {{ formatDate(processInstance.startTime) }} {{ t('process.instance.submitTime') }} </div>
        </div>

        <el-tabs v-model="activeTab">
          <!-- 表单信息 -->
          <el-tab-pane :label="t('process.instance.approvalDetail')" name="form">
          ...
          <!-- 流程图 -->
          <el-tab-pane :label="t('process.instance.flowDiagram')" name="diagram">
            <div class="form-scroll-area">
          ...
          <!-- 流转记录 -->
          <el-tab-pane :label="t('process.instance.flowRecord')" name="record">
            <div class="form-scroll-area">
              <el-scrollbar>
...
const getApprovalDetail = async () => {
  ...
  try {
    ...
    message.error(t('process.instance.queryError') || '查询审批详情失败：流程实例不存在或已被清理')

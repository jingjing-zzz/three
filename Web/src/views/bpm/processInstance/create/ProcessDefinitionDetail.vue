<template>
  <ContentWrap :bodyStyle="{ padding: '10px 20px 0' }">
    <div class="processInstance-wrap-main">
      <el-scrollbar>
        <div class="text-#878c93 h-15px">{{ t('process.task.processName') }}：{{ selectProcessDefinition.name }}</div>
        <el-divider class="!my-8px" />

        <!-- 中间主要内容 tab 栏 -->
        <el-tabs v-model="activeTab">
          <!-- 表单信息 -->
          <el-tab-pane :label="t('approval.fillForm')" name="form">
        ...
        <div class="b-t-solid border-t-1px border-[var(--el-border-color)]">
          ...
          >
            <el-button plain type="success" @click="submitForm">
              <Icon icon="ep:select" />&nbsp; {{ t('process.instance.submit') }}
            </el-button>

<template>
  <el-button plain @click="handleQuery"> <Icon icon="ep:refresh" class="mr-5px" /> {{ t('common.refresh') }} </el-button>
  <el-button
    type="primary"
    plain
  ...
  >
    ...
    <el-table-column
      :label="t('limitConfig.ruleUserGroup')"
      align="center"
      :formatter="(row) => row.users?.map((user: any) => user.nickname).join('，')"
    />
    <el-table-column
      :label="t('limitConfig.ruleDeptGroup')"
      align="center"
      :formatter="(row) => row.depts?.map((dept: any) => dept.name).join('，')"
    />
    <el-table-column
      :label="
        confType === LimitConfType.CUSTOMER_QUANTITY_LIMIT ? t('limitConfig.customerQuantityLimit') : t('limitConfig.customerLockLimit')
      "
      align="center"
      prop="maxCount"
    />
    <el-table-column
      ...
      :label="t('limitConfig.dealCountEnabled')"
      align="center"

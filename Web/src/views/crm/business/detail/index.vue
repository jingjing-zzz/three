<template>
  ...
  <el-col>
    <el-tabs>
      <el-tab-pane :label="t('crm.business.followUpTab')">
        <FollowUpList :biz-id="businessId" :biz-type="BizTypeEnum.CRM_BUSINESS" />
      </el-tab-pane>
      <el-tab-pane :label="t('crm.business.basicInfoTab')">
        <BusinessDetailsInfo :business="business" />
      </el-tab-pane>
      <el-tab-pane :label="t('crm.business.contactTab')" lazy>
        <ContactList
          :biz-id="business.id!"
      ...
      <el-tab-pane :label="t('crm.business.teamMemberTab')">
        <PermissionList
...
onMounted(async () => {
  if (!params.id) {
    message.warning(t('crm.business.paramError'))
    close()
    return
  }

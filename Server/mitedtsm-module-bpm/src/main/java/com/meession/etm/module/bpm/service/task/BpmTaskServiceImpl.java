...
import com.meession.etm.framework.common.util.date.DateUtils;
...
import com.meession.etm.framework.common.util.object.PageUtils;
...
import com.meession.etm.module.bpm.convert.task.BpmTaskConvert;
...
import com.meession.etm.module.bpm.enums.definition.*;
import com.meession.etm.module.bpm.enums.task.BpmCommentTypeEnum;
import com.meession.etm.module.bpm.enums.task.BpmReasonEnum;
import com.meession.etm.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import com.meession.etm.module.bpm.framework.flowable.core.util.BpmHttpRequestUtils;
...
import com.meession.etm.module.bpm.service.message.BpmMessageService;
...
import static com.meession.etm.module.bpm.framework.flowable.core.enums.BpmnModelConstants.START_USER_NODE_ID;
import static com.meession.etm.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    public PageResult<HistoricTaskInstance> getTaskDonePage(Long userId, BpmTaskPageReqVO pageVO) {
        ...
        // 执行查询，先获取所有数据再进行内存过滤和分页
        List<HistoricTaskInstance> allTasks = taskQuery.list();
        
        // 特殊：强制移除自动完成的“发起人”节点
        allTasks.removeIf(task -> task.getTaskDefinitionKey().equals(START_USER_NODE_ID));
        // TODO @芋艿：https://t.zsxq.com/MNzqp 【flowable bug】：taskCreatedAfter、taskCreatedBefore 拼接的是 OR
        if (ArrayUtil.isNotEmpty(pageVO.getCreateTime())) {
            allTasks.removeIf(task -> task.getCreateTime() == null
        ...
        long count = allTasks.size();
        if (count == 0) {
            return PageResult.empty();
        }
        // 手动分页
        int start = PageUtils.getStart(pageVO);
        int end = Math.min(start + pageVO.getPageSize(), (int) count);
        List<HistoricTaskInstance> tasks = allTasks.subList(start, end);
        return new PageResult<>(tasks, count);
    }
...
//        }
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 mitedtsm-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/mitedtsm-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/mitedtsm-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/mitedtsm-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
